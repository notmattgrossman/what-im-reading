...
/**
 * Create or update the tag suggestions datalist
 * @returns {string} The datalist ID to use with inputs
 */
function createTagSuggestionsDatalist() {
    ...
    // Add all existing tags as options
...
function createTagInput(value = '', container, onInput) {
    ...
    // Add autocomplete with existing tags
    const datalistId = createTagSuggestionsDatalist();
...
async function saveArticleChanges(articleIndex, newTitle, newDate, newFavorite, newReview, newTags) {
    ...
    try {
        ...
        // Update tag suggestions after saving
        createTagSuggestionsDatalist();
...
async function loadArticles() {
    ...
    try {
        ...
        // Update tag suggestions with loaded articles
        createTagSuggestionsDatalist();
...
function applyFilters() {
    ...
    // Scroll to top of articles when filters are applied
...
function initializeApp() {
    ...
    if (addButton) {
        ...
        // Add precise hover detection for color change
        addButton.addEventListener('mouseenter', () => {
            addButton.addEventListener('mousemove', handleButtonHover);
            addButton.addEventListener('mouseleave', handleButtonLeave);
        });
        ...
        function handleButtonHover(e) {
            ...
            // Check if mouse is within the actual 48x48 button area
            if (x >= 0 && x <= 48 && y >= 0 && y <= 48) {
                addButton.classList.add('button-hover');
            } else {
                addButton.classList.remove('button-hover');
            }
        }
        
        function handleButtonLeave() {
            addButton.classList.remove('button-hover');
            addButton.removeEventListener('mousemove', handleButtonHover);
            addButton.removeEventListener('mouseleave', handleButtonLeave);
        }
    }
